...
export class DocumentViewerComponent implements OnInit, OnDestroy {
  ...
  cargarDetalle(): void {
    ...
    this.documentosService.getDocumentoDetails(this.documentoId).subscribe({
      next: (doc) => {
        ...
        this.documentosService.getPermiso(this.documentoId).subscribe(res => {
          ...
          if (this.esDocx()) {
            ...
            this.pdfUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`/api/documentos/${this.documentoId}/download?token=${token}`);
  ...
  onEditorCreated(): void {
    // Si se crea el editor y ya tenemos los datos, cargamos el documento
    if (this.documento && !this.loading()) {
      this.cargarDocumentoEnEditor();
    }
  }
  ...
  cargarDocumentoEnEditor(): void {
    ...
    this.documentosService.downloadDocumento(this.documentoId).subscribe({
      next: (blob: Blob) => {
        // Enviar el docx a Syncfusion Import API para convertirlo en SFDT JSON
          ...
          next: (sfdt: string) => {
            setTimeout(() => {
              if (this.documentEditorContainer && this.documentEditorContainer.documentEditor) {
                ...
                // Habilitar o deshabilitar edición en base a permisos dinámicos
                if (this.permiso() === 'view') {
                  this.documentEditorContainer.documentEditor.isReadOnly = true;
                } else {
                  this.documentEditorContainer.documentEditor.isReadOnly = false;
                }
              }
              this.loading.set(false);
            }, 100);
      ...
      error: (err) => {
        console.error('Error cargando binario en Syncfusion:', err);
        this.loading.set(false);
      }
    });
  }
  ...
  recargarDocumento(): void {
    this.documentosService.getDocumentoDetails(this.documentoId).subscribe(doc => {
      this.documento = doc;
      if (this.esDocx()) {
        this.cargarDocumentoEnEditor();
      }
    ...
    this.documentEditorContainer.documentEditor.saveAsBlob('Docx').then((blob: Blob) => {
      ...
      this.documentosService.subirNuevaVersion(this.documentoId, blob, comentario || 'Edición colaborativa', this.documento?.nombre).subscribe({
        next: (updatedDoc) => {
          ...
          // Notificar vía socket al grupo de editores
  ...
  // Notificar a otros usuarios mientras se escribe
